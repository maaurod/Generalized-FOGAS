    ...
    for t in range(1, max(STEP_CHECKPOINTS) + 1):
        ...
        if t in STEP_CHECKPOINTS:
            ...
            row = {
                ...
                "greedy_success_rate": eval_stats["greedy_success_rate"],
...
def normalize_workers(workers: int) -> int:
    if workers < 0:
        raise ValueError("--workers must be >= 0. Use 0 for all available CPUs.")
    if workers == 0:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Number of parallel worker processes. Use 0 for all available CPUs.",
    )
...
def main() -> None:
    ...
    print("Ranking       : avg_return desc, greedy_success_rate desc, avg_solve_steps asc")
    ...
    results_df = results_df.sort_values(
        ["avg_return", "greedy_success_rate", "avg_solve_steps", "objective"],
