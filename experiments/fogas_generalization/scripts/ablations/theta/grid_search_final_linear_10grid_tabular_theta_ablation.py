...
RESULTS_DIR = (
    PROJECT_ROOT / "data" / "results" / "generalization" / "ablations" / "theta" / "10grid"
)

DATASET_PATH = DATASETS_DIR / "10grid_tabular_new.csv"
OUTPUT_CSV = RESULTS_DIR / "final_linear_10grid_tabular_theta_ablation.csv"
BEST_CSV = RESULTS_DIR / "final_linear_10grid_tabular_theta_ablation_best.csv"
STATS_CSV = RESULTS_DIR / "final_linear_10grid_tabular_theta_ablation_stats.csv"
CURVES_CSV = RESULTS_DIR / "final_linear_10grid_tabular_theta_ablation_curves.csv"
CURVE_STATS_CSV = RESULTS_DIR / "final_linear_10grid_tabular_theta_ablation_curve_stats.csv"
...
THETA_LAMBDA_GRID = [1e-9, 3e-9, 1e-8, 3e-8, 1e-7, 3e-7, 1e-6, 3e-6, 1e-5, 3e-5, 1e-4]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run FinalLinearSolver theta ablations on the deterministic 10x10 tabular grid."
    )
...
def make_solver(candidate, device, seed):
...
def make_candidate(
    ...
    return {
        ...
        "theta_optimizer": "adam",
        "theta_start_mode": str(theta_start_mode),
        "theta_include_beta_cov": False,
...
def all_candidates():
    candidates = []
    for seed, theta_start_mode in itertools.product(SWEEP_SEEDS, THETA_START_MODE_GRID):
        candidates.append(
            make_candidate(
...
def run_candidate(candidate, mdp, planner, device, d_star, v_star, save_curves=True):
    ...
    curve_rows = []
    try:
        solver = make_solver(candidate, device=device, seed=int(candidate["seed"]))
...
def build_curve_stats_frame(curve_results):
    ...
    if df.empty:
        return df
    return df.sort_values(
        ["ablation", "theta_mode", "theta_start_mode", "theta_lambda", "D_theta", "theta_inner_steps", "checkpoint_step"],
        ascending=True,
        na_position="last",
    )
...
def run_grid_search():
    ...
    desc = "FinalLinearSolver 10-grid tabular theta ablation"
