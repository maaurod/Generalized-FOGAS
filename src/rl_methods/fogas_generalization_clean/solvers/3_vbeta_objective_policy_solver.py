...
VBetaSolver = import_module(".2_vbeta_solver", __package__).VBetaSolver


class VBetaObjectivePolicySolver(VBetaSolver):
    """
    Vectorized beta-parameter FOGAS variant with the policy update induced by
    the reparametrized empirical objective.

    The beta/theta updates are inherited from the vectorized beta solver logic,
    while the policy is updated as a tabular KL mirror-ascent step:

        pi_{t+1}(a|x) proportional to pi_t(a|x) exp(alpha G_t(x,a)).
    """
    ...
    def run(
        ...
        theta_bar_history = []
        diagnostics_history = []
        pi_mat = torch.ones((N, A), dtype=torch.float64, device=device) / A
        ...
        for t in iterator:
            ...
            log_pi_next = torch.log(torch.clamp(pi_mat, min=1e-300)) + alpha * G
            pi_next = self._row_softmax(log_pi_next)
            ...
            diagnostics = {
                ...
                "policy_mean_entropy": float(self._policy_entropy(pi_mat).detach().cpu().item()),
                ...
                "negative_state_weight_count": state_weight_signs["negative_count"],
                "positive_state_weight_count": state_weight_signs["positive_count"],
                "zero_state_weight_count": state_weight_signs["zero_count"],
                "shown_negative_state_weight_states": state_weight_signs["shown_negative_states"],
                "shown_positive_state_weight_states": state_weight_signs["shown_positive_states"],
                "shown_zero_state_weight_states": state_weight_signs["shown_zero_states"],
                "state_weight_signs": state_weight_signs["signs"],
            ...
            if verbose and (t % log_interval == 0):
                print(
                    f"[FOGAS objective-policy] Iter {t+1}/{T} "
